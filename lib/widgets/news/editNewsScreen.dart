...
class _EditNewsState extends State<EditNews> {
  ...
  Future<void> initializeNewsForEditing() async {
    try {
      ...
      if (newsDoc.exists) {
        ...
        final newsContent = newsData['newsContent'] as String;
        final newsLogoUrl = newsData['newsLogo'] as String;

        // Fetch the image data and set it to _imageData
        final imageBytes = await _getImageData(newsLogoUrl);

        setState(() {
          newsContentController.text = newsContent;
          newImage = newsLogoUrl;
      ...
      print('Error initializing news for editing: $error');
    }
  }
  ...
  Future<Uint8List> _getImageData(String imageUrl) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Failed to retrieve image: ${response.statusCode}');
        return Uint8List(0); // Return an empty Uint8List
      }
    } catch (error) {
  ...
  void createNews() {
    ...
    FirebaseFirestore.instance
      ...
      _showSnackBar('News updated successfully', Colors.green);
      Navigator.pop(context);
    }).catchError((error) {
      print('Failed to updated News: $error');
      _showSnackBar('Failed to updated News', Colors.red);
    });
  }
