...
import 'matches/finalRoundMatches.dart';
import 'matches/secondRoundMatches.dart';

class MatchOrganizerButton extends StatelessWidget {
  final String tournamentId;

  MatchOrganizerButton(
      {required this.tournamentId, required BuildContext context});

  DateTime randomDateInTheNextTenDays() {
    final today = DateTime.now();
    final randomDays = Random().nextInt(10);
    final randomDate = today.add(Duration(days: randomDays));
    return randomDate;
  }
  ...
  Future<List<String>> _fetchRefereeNames() async {
    ...
    List<String> refereeDocIds = List<String>.from(
        (eventDoc.data() as Map<String, dynamic>)['refereesDocIds'] ?? []);

    for (String id in refereeDocIds) {
      DocumentSnapshot refDoc =
          await firestore.collection('referees').doc(id).get();
      if (refDoc.exists) {
        String name = (refDoc.data() as Map<String, dynamic>)['name'] ?? '';
  ...
  Future<void> organizeAutomaticMatches() async {
    final firestore = FirebaseFirestore.instance;

    // Fetch teams that have requested to join the given tournamentId
    final teamsQuery = await firestore
        .collection('requestToJoin')
        .where('tournamentId', isEqualTo: tournamentId)
        .get();

    if (teamsQuery.docs.length != 8) {
      print('The number of teams is not 8. Cannot organize matches.');
      return;
    }

    // Extract the team IDs
    List<String> teamIds =
        teamsQuery.docs.map((doc) => doc['teamId'] as String).toList();
    teamIds.shuffle();

    // Fetch available referees
    List<String> availableReferees = await _fetchRefereeNames();
    if (availableReferees.isEmpty) {
      print('No available referees. Cannot organize matches.');
      return;
    }
    ...
    for (int i = 0; i < 8; i += 2) {
      ...
      // Select random time and referee for the match
      String selectedTime =
      ...
      print(
          'Match scheduled: $team1 vs $team2 on $formattedDate at $selectedTime refereed by $selectedReferee');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Row(
          ...
          children: [
            Text(
              'Matches',
              style: TextStyle(
                  fontFamily: 'karla', fontSize: 26, color: Color(0xff6858FE)),
            ),
            Row(
              children: [
                Container(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () {
                      // organizeAutomaticMatches();
                      showDialog(
                        context: context,
                        builder: (BuildContext context) {
                          return MatchOrganizerDialog(
                ...
                Container(
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () {
                      ...
                      //   context: context,
                      //   builder: (BuildContext context) {
                      //     return MatchOrganizerDialog(tournamentId: tournamentId);
                      //   },
                      // );
                    },
                    child: Text(
        ...
        // ),
        SizedBox(height: 20),
        MatchScheduleWidget(tournamentId: tournamentId),
        SizedBox(
          height: 20,
        ),
        SecondRoundMatchScheduleWidget(tournamentId: tournamentId),
        SizedBox(height: 20),
        FinalRoundMatchScheduleWidget(tournamentId: tournamentId),
      ],
    );
  }
}

class MatchOrganizerDialog extends StatefulWidget {
  final String tournamentId;

  MatchOrganizerDialog({required this.tournamentId});

  @override
  _MatchOrganizerDialogState createState() => _MatchOrganizerDialogState();
}

class _MatchOrganizerDialogState extends State<MatchOrganizerDialog> {
  String? team1Id;
  String? team2Id;
  String? selectedReferee;
  ...
  void initState() {
    ...
    matchTimeController = TextEditingController();
  ...
  void organizeMatch(String selectedTime) {
...
// get referees name in dropdowns
...
class MatchScheduleWidget extends StatelessWidget {
  final String tournamentId;

  MatchScheduleWidget({required this.tournamentId});
  ...
  void showUpdateMatchDialog(
    ...
    showDialog(
      context: context,
      builder: (context) => MatchUpdaterDialog(
        tournamentId: tournamentId,
        matchId: matchId,
        team1Id: team1Id,
        team2Id: team2Id,
                        ...
                        FutureBuilder<QuerySnapshot>(
                          ...
                          builder: (context, snapshot) {
                            if (snapshot.connectionState ==
                                ...
                                snapshot.hasData) {
                              final team1Data = snapshot.data!.docs.first.data()
                                  as Map<String, dynamic>;
                              final team1Name =
                                  team1Data['teamName'] ?? 'Unknown';
                              final team1Logo = team1Data['logo'] ?? 'Unknown';
                              ...
                              return FutureBuilder<QuerySnapshot>(
                                ...
                                builder: (context, team2Snapshot) {
                                  if (team2Snapshot.connectionState ==
                                    ...
                                    final team2Data =
                                        team2Snapshot.data!.docs.first.data()
                                            as Map<String, dynamic>;
                                    final team2Name =
...
// to edit match
class MatchUpdaterDialog extends StatefulWidget {
  final String tournamentId;
  ...
  @override
  _MatchUpdaterDialogState createState() => _MatchUpdaterDialogState();
}

class _MatchUpdaterDialogState extends State<MatchUpdaterDialog> {
  late TextEditingController matchNumberController;
  late TextEditingController matchDateController;
  late TextEditingController matchTimeController;
  late TextEditingController groundController;
 ...
 // TextField(
  ...
  // AlertDialog(
    ...
    //         TextField(
    //           controller: groundController,
    //           decoration: InputDecoration(labelText: 'Ground'),
    //         ),
    //       ],
    //     ),
    //   ),
